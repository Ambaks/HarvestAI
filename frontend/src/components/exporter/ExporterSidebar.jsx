      ...
      <ul className="space-y-3 mt-20">
        <li className="space-y-6">
            <SidebarButton 
              ...
              text="Dashboard" 
              active={location.pathname === "/exporter-dashboard" || location.pathname === "/exporter-dashboard/"}
            />

            <SidebarButton 
              to={`/exporter-dashboard/mycrops/${user.id}`} 
              icon={<GiWheat className="text-[18px]" />}
              text="My Crops" 
              active={location.pathname.includes("/exporter-dashboard/mycrops")}
            />

            <SidebarButton 
              to={`/exporter-dashboard/transactions/${user.id}`} 
              icon={<FaMoneyCheck className="text-[18px]" />}
              text="Sales" 
              active={location.pathname.includes("/exporter-dashboard/transactions")}
            />

            <SidebarButton 
              to="/exporter-dashboard/marketplace" 
              icon={<TbBuildingCircus className="text-[18px]" />}
              text="Marketplace" 
              active={location.pathname.includes("/exporter-dashboard/marketplace")}
            />

            <SidebarButton 
              to={`/exporter-dashboard/settings/${user.id}`} 
              icon={<RxGear className="text-[18px]" />}
              text="Settings" 
              active={location.pathname.includes("/exporter-dashboard/settings")}
            />
        </li>
      </ul>
    </div>
