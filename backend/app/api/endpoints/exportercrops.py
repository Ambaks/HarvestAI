...
def get_crops_for_exporter(exporter_id: str, db: Session = Depends(get_db)):
...
def update_crop_status(exporter_crop_id: str, update_data: ExporterCropUpdate, db: Session = Depends(get_db)):
...
def delete_crop_assignment(exporter_crop_id: str, db: Session = Depends(get_db)):
